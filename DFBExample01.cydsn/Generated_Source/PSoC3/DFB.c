...
static void DFB_LoadReg32(reg32 regAddress[], const uint32 source[], uint8 count);
static void DFB_ZeroReg32(reg32 regAddress[], uint8 count);

uint8 DFB_initVar = 0u;


/*******************************************************************************
* Function Name: DFB_LoadReg32
********************************************************************************
* 
* Summary:
...
void DFB_Init(void) 
{
    ...
    /* Turns off Run Bit */
    DFB_CR_REG &= (uint8) (~DFB_RUN_MASK);
    /* Powers up the DFB RAMS */
    DFB_RAM_EN_REG = DFB_RAM_DIR_BUS;
    /* Puts DFB RAM on the bus */
    DFB_RAM_DIR_REG = DFB_RAM_DIR_BUS;

    /* Clears all DFB RAM */
    DFB_ZeroReg32(DFB_DA_RAM_PTR, DFB_DA_RAM_SIZE);
    DFB_ZeroReg32(DFB_DB_RAM_PTR, DFB_DB_RAM_SIZE);
    DFB_ZeroReg32(DFB_CSA_RAM_PTR, DFB_CSA_RAM_SIZE);
    DFB_ZeroReg32(DFB_CSB_RAM_PTR, DFB_CSB_RAM_SIZE);
    DFB_ZeroReg32(DFB_ACU_RAM_PTR, DFB_ACU_RAM_SIZE);
    DFB_ZeroReg32(DFB_CFSM_RAM_PTR, DFB_CFSM_RAM_SIZE);

    /* Writes DFB RAMs */
    /* Control Store RAMs */
    #if (DFB_OPTIMIZE_ASSEMBLY)
        DFB_LoadReg32(DFB_CSA_RAM_PTR, DFB_cstoreA,
                                    DFB_CSA_RAM_SIZE_CUR);
        DFB_LoadReg32(DFB_CSB_RAM_PTR, DFB_cstoreB,
                                    DFB_CSB_RAM_SIZE_CUR);
    #else
        DFB_LoadReg32(DFB_CSA_RAM_PTR, DFB_control,
                                    DFB_CSA_RAM_SIZE_CUR);
        DFB_LoadReg32(DFB_CSB_RAM_PTR, DFB_control,
                                    DFB_CSB_RAM_SIZE_CUR);
    #endif /* DFB_OPTIMIZE_ASSEMBLY */

    /* Loads initial data to Data RAMs */
    DFB_LoadReg32(DFB_DA_RAM_PTR, DFB_data_a, DFB_DA_RAM_SIZE_CUR);
    DFB_LoadReg32(DFB_DB_RAM_PTR, DFB_data_b, DFB_DB_RAM_SIZE_CUR);
    /* Loads initial data to ACU RAM */
    DFB_LoadReg32(DFB_ACU_RAM_PTR, DFB_acu, DFB_ACU_RAM_SIZE_CUR);
    /* Loads initial data to CFSM RAM */
    DFB_LoadReg32(DFB_CFSM_RAM_PTR, DFB_cfsm,
                                DFB_CFSM_RAM_SIZE_CUR);

    /* Takes DFB RAM off the bus */
    DFB_RAM_DIR_REG = DFB_RAM_DIR_DFB;

    /* Sets up interrupt and DMA events */
    DFB_SetInterruptMode(DFB_INIT_INTERRUPT_MODE);
    DFB_SetDMAMode(DFB_INIT_DMA_MODE);

    /* Sets up internal signals, which will be mapped to the DSI outputs */
    DFB_SetOutput1Source(DFB_INIT_OUT1_SOURCE);
    DFB_SetOutput2Source(DFB_INIT_OUT2_SOURCE);
...
void DFB_Enable(void) 
{
    ...
    DFB_RAM_EN_REG = DFB_RAM_DIR_BUS;
    /* Turns on Run Bit */
    DFB_CR_REG |= DFB_RUN_MASK;
}


/*******************************************************************************
* Function Name: DFB_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  DFB_initVar - used to check initial configuration, modified on
*  first function call.
*
* Reentrant:
*  No.
...
void DFB_Stop(void) 
{
    ...
    /* Powers off DFB */
    DFB_PM_ACT_CFG_REG &= (uint8) (~DFB_PM_ACT_MSK);
    /* Disables DFB block in Alternate Active (Standby) mode */
    DFB_PM_STBY_CFG_REG &= (uint8) (~DFB_PM_STBY_MSK);
...
/*******************************************************************************
* Function Name: DFB_SetCoherency
********************************************************************************
*
* Summary:
...
*    Define                             Description
*     DFB_STGA_KEY_LOW      Key Byte is low byte.
*     DFB_STGA_KEY_MID      Key Byte is med byte.
*     DFB_STGA_KEY_HIGH     Key Byte is high byte.
*   Sets the Key Coherency Byte of the Staging B register
*    Define                             Description
*     DFB_STGB_KEY_LOW      Key Byte is low byte.
*     DFB_STGB_KEY_MID      Key Byte is med byte.
*     DFB_STGB_KEY_HIGH     Key Byte is high byte.
*   Sets the Key Coherency Byte of the Holding A register
*    Define                             Description
*     DFB_HOLDA_KEY_LOW     Key Byte is low byte.
*     DFB_HOLDA_KEY_MID     Key Byte is med byte.
*     DFB_HOLDA_KEY_HIGH    Key Byte is high byte.
*   Sets the Key Coherency Byte of the Holding B register
*    Define                             Description
*     DFB_HOLDB_KEY_LOW     Key Byte is low byte.
*     DFB_HOLDB_KEY_MID     Key Byte is med byte.
*     DFB_HOLDB_KEY_HIGH    Key Byte is high byte.
*
* Return:
*  None.
*
...
{
    DFB_COHER_REG = coherencyKeyByte;
}


/*******************************************************************************
* Function Name: DFB_SetDalign
********************************************************************************
*
* Summary:
...
*     DFB_STGA_DALIGN_HIGH    Writes shifted left by 8-bits.
*   Shifts the write left by a byte.
*    Define                               Description
*     DFB_STGB_DALIGN_LOW     Writes normally.
*     DFB_STGB_DALIGN_HIGH    Writes shifted left by 8-bits.
*   Shifts the read right by a byte.
*    Define                               Description
*     DFB_HOLDA_DALIGN_LOW     Reads normally.
*     DFB_HOLDA_DALIGN_HIGH    Reads shifted right by 8-bits.
*   Shifts the read right by a byte.
*    Define                               Description
*     DFB_HOLDB_DALIGN_LOW     Reads normally.
*     DFB_HOLDB_DALIGN_HIGH    Reads shifted right by 8-bits.
*
* Return:
*  None.
*
...
{
    DFB_DALIGN_REG = dalignKeyByte;
}


/*******************************************************************************
* Function Name: DFB_LoadDataRAMA
********************************************************************************
*
* Summary:
...
*  DFB_ADDRESS_OUT_OF_RANGE - error code, indicates that the
*  address is out of range.
*  DFB_DATA_OUT_OF_RANGE - error code, indicates a data overflow
*  error.
*
* Side Effects:
...
{
    uint8 result = DFB_SUCCESS;
    uint8 i = 0u;
    
    if (((& addr[0u]) < DFB_DA_RAM_PTR) || ((& addr[0u]) > DFB_DA_RAM_LAST_ELEM_PTR))
    {
        result = DFB_ADDRESS_OUT_OF_RANGE;
    }
    else if ((size == 0u) || ((& addr[size - 1u]) > DFB_DA_RAM_LAST_ELEM_PTR))
    {
        result = DFB_DATA_OUT_OF_RANGE;
    }
    else
    {
        /* Puts DFB RAM on the bus */
        DFB_RAM_DIR_REG |= DFB_RAM_DPA_DIR;
        while (size > 0u)
        {
        ...
        /* Takes DFB RAM off the bus */
        DFB_RAM_DIR_REG &= (uint8) (~DFB_RAM_DPA_DIR);
    }
...
/*******************************************************************************
* Function Name: DFB_LoadDataRAMB
********************************************************************************
*
* Summary:
...
*  DFB_ADDRESS_OUT_OF_RANGE - error code, indicates that the
*  address is out of range.
*  DFB_DATA_OUT_OF_RANGE - error code, indicates data overflow
*  error.
*
* Side Effects:
*  This API doesn't stop the DFB if it is already started. Hence, recommended
     ...
     
{
    ...
    if (((& addr[0u]) < DFB_DB_RAM_PTR) || ((& addr[0u]) > DFB_DB_RAM_LAST_ELEM_PTR))
    {
        result = DFB_ADDRESS_OUT_OF_RANGE;
    }
    else if ((size == 0u) || ((& addr[size - 1u]) > DFB_DB_RAM_LAST_ELEM_PTR))
    {
        result = DFB_DATA_OUT_OF_RANGE;
    }
    else
    {
        /* Puts DFB RAM on the bus */
        DFB_RAM_DIR_REG |= DFB_RAM_DPB_DIR;
        while (size > 0u)
        {
            CY_SET_REG32((reg32 *) (& addr[i]), (uint32) ptr[i]);
            size--;
        ...
        /* Takes DFB RAM off the bus */
        DFB_RAM_DIR_REG &= (uint8) (~DFB_RAM_DPB_DIR);
    }
...
/*******************************************************************************
* Function Name: DFB_LoadInputValue
********************************************************************************
*
* Summary:
...
*
* Parameters:
*  channel: Use either DFB_CHANNEL_A (1) or
*           DFB_CHANNEL_B (0) asarguments to the function;
...
void DFB_LoadInputValue(uint8 channel, int32 sample) 
{
    ...
    if (channel == DFB_CHANNEL_A)
    {
        DFB_STAGEA_REG  = (uint8) value;
        DFB_STAGEAM_REG = (uint8) (value >> 8u);
        DFB_STAGEAH_REG = (uint8) (value >> 16u);
    }
    else if (channel == DFB_CHANNEL_B)
    {
        DFB_STAGEB_REG  = (uint8) value;
        DFB_STAGEBM_REG = (uint8) (value >> 8u);
        DFB_STAGEBH_REG = (uint8) (value >> 16u);
    }
    else
    {
...
/*******************************************************************************
* Function Name: DFB_GetOutputValue
********************************************************************************
*
* Summary:
...
* Parameters:
*  channel: Use either DFB_CHANNEL_A (1) or
*           DFB_CHANNEL_B (0) as arguments to the function.
*
* Return:
...
int32 DFB_GetOutputValue(uint8 channel) 
{
    ...
    {
        data0 = DFB_HOLDA_REG;
        data1 = DFB_HOLDAM_REG;
        data2 = DFB_HOLDAH_REG;
        data3 = ((data2 & 0x80u) == 0x80u) ? 0xFFu : 0x00u;
    }
    else if (channel == DFB_CHANNEL_B)
    {
        data0 = DFB_HOLDB_REG;
        data1 = DFB_HOLDBM_REG;
        data2 = DFB_HOLDBH_REG;
        data3 = ((data2 & 0x80u) == 0x80u) ? 0xFFu : 0x00u;
    }
    else
    {
...
/*******************************************************************************
* Function Name: DFB_SetInterruptMode
********************************************************************************
*
* Summary:
...
*          interrupts.
*    Define                    Description
*     DFB_HOLDA    Interrupt is generated each time new valid data
*                               is written into the output Holding register A;
*     DFB_HOLDB    Interrupt is generated each time new valid data
*                               is written into the output Holding register B;
*     DFB_SEMA0    Interrupt is generated each time a '1' is
*                               written into the semaphore register bit 0;
*     DFB_SEMA1    Interrupt is generated each time a '1' is
*                               written into the semaphore register bit 1;
*     DFB_SEMA2    Interrupt is generated each time a '1' is
...
{
    DFB_INT_CTRL_REG = events & DFB_EVENT_MASK;
}


/*******************************************************************************
* Function Name: DFB_GetInterruptSource
********************************************************************************
*
* Summary:
*  This function looks at the DFB_SR register to see which
...
*  DFB interrupt.
*    Define                    Description
*     DFB_HOLDA    Holding register A is a source of the current
*                               interrupt;
*     DFB_HOLDB    Holding register B is a source of the current
*                               interrupt;
*     DFB_SEMA0    Semaphore register bit 0 is a source of the
*                               current interrupt;
*     DFB_SEMA1    Semaphore register bit 1 is a source of the
*                               current interrupt;
*     DFB_SEMA2    Semaphore register bit 2 is a source of the
*                               current interrupt.
*
*******************************************************************************/
uint8 DFB_GetInterruptSource(void) 
{
    ...
    return ((uint8) (DFB_SR_REG >> DFB_SR_EVENT_SHIFT));
}


/*******************************************************************************
* Function Name: DFB_ClearInterrupt
********************************************************************************
*
* Summary:
...
*    Define                    Description
*     DFB_HOLDA    Clear interrupt from Holding register A.
*                               (Reading the Holding register A also clears this
*                                bit);
*     DFB_HOLDB    Clear interrupt from Holding register B.
...
{
    DFB_SR_REG &= (uint8) (interruptMask << DFB_SR_EVENT_SHIFT);
}


/*******************************************************************************
* Function Name: DFB_SetDMAMode
********************************************************************************
*
* Summary:
...
*   DMA Request 1:
*    Define                                 Description
*    DFB_DMAREQ1_DISABLED       No request will be generated;
*    DFB_DMAREQ1_HOLDA          Output value ready in the holding
                                             register on channel A;
*    DFB_DMAREQ1_SEM0           Semaphore 0;
*    DFB_DMAREQ1_SEM1           Sempahore 1.
*   DMA Request 2:
*    Define                                 Description
*    DFB_DMAREQ2_DISABLED       No request will be generated;
*    DFB_DMAREQ2_HOLDB          Output value ready in the holding
                                             register on channel B;
*    DFB_DMAREQ2_SEM0           Semaphore 0;
*    DFB_DMAREQ2_SEM1           Sempahore 1.
*
* Return:
*  None.
*
...
{
    DFB_DMA_CTRL_REG = events & DFB_DMA_CTRL_MASK;
}


/*******************************************************************************
* Function Name: DFB_SetSemaphore
********************************************************************************
*
* Summary:
...
{
    DFB_SEM_REG = (uint8) ((mask & DFB_SEM_MASK) << DFB_SEM_ENABLE_SHIFT) |
                                 DFB_SEM_MASK;
}


/*******************************************************************************
*  Function Name: DFB_GetSempahores
********************************************************************************
*
* Summary:
...
uint8 DFB_GetSemaphores(void) 
{
    ...
    return (DFB_SEM_REG & DFB_SEM_MASK);
}


/*******************************************************************************
*  Function Name: DFB_ClearSemaphore
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DFB_SetOutput1Source
********************************************************************************
*
* Summary:
...
*   Signal                      Description
*    DFB_DFB_RUN    DFB RUN Bit. This is the same bit as the RUN
*                                bit in the DFB_CR register.
*    DFB_SEM0       Semaphore Bit 0.
*    DFB_SEM1       Semaphore Bit 1.
*    DFB_DFB_INTR   DFB Interrupt. This is the same signal as the
...
{
    DFB_DSI_CTRL_REG = (DFB_DSI_CTRL_REG & DFB_DFB_GBL1_OUT_MASK) | source;
}


/*******************************************************************************
* Function Name: DFB_SetOutput2Source
********************************************************************************
*
* Summary:
*  This function allows you to choose which internal signal will be mapped to
...
*    DFB_DPSIGN     Datapath Sign. This signal asserts anytime the
*                                 output of the ALU in the Datapath unit is
*                                 negative. It will remain high for each cycle
*                                 this condition is true.
*    DFB_DPTHRESH   Datapath Threshold Crossed. This signal
...
*    DFB_DPEQ       Datapath ALU=0. This signal asserts high when
...
{
    DFB_DSI_CTRL_REG = (DFB_DSI_CTRL_REG & DFB_DFB_GBL2_OUT_MASK) | source;
}
